...
sudo sed -i "s/EXAMPLE\.COM/maruthiprithivi\.sg/" /var/kerberos/krb5kdc/kdc.conf
...
sudo sed -i "s/^# \.example\.com = EXAMPLE\.COM/\.example\.com = maruthiprithivi\.sg/" /etc/krb5.conf \
&& sudo sed -i "s/^# example\.com = EXAMPLE\.COM/\example\.com = maruthiprithivi\.sg/" /etc/krb5.conf

sudo sed -i "s/^# EXAMPLE\.COM/ maruthiprithivi\.sg/" /etc/krb5.conf \
...
sudo sed -i "s/^# default_realm = EXAMPLE.COM/default_realm = maruthiprithivi\.sg/" /etc/krb5.conf
...
sudo echo "*/admin@maruthiprithivi\.sg *" > /var/kerberos/krb5kdc/kadm5.acl
sudo echo "cloudera-scm@maruthiprithivi\.sg admilc" >> /var/kerberos/krb5kdc/kadm5.acl
...
sudo echo "cloudera-scm@maruthiprithivi\.sg" > /etc/cloudera-scm-server/cmf.principal
